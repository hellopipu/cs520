...
def generate_random_matrix(dim, p, cnt_initial_fire=10):
    ...
    while 1:
        ...
        while (len(fire_point_list) != cnt_initial_fire):
            ...
            fire_point = None
            while not DFS_search(maze_matrix_fire, fire_point, ignore_fire=True) or not DFS_search(maze_matrix_fire,
                                                                                                   [dim - 1, dim - 1]):
                #### use DFS to validate the generated maze with fire
                maze_matrix_fire = copy.deepcopy(maze_matrix)
                while fire_point is None or fire_point == [0, 0] or fire_point == [dim - 1, dim - 1] or maze_matrix[
                    fire_point[0], fire_point[1]] != 2:
...
def search_valid(maze_matrix):
    ...
    return DFS_search(maze_matrix, [dim - 1, dim - 1]) and DFS_search(maze_matrix, list(fire_point_array[0]),
                                                                      ignore_fire=True)
...
def generate_cost_map(matrix_, fire_point_list, q):
    ...
    cost_map = cost_map.max() - cost_map
...
def Astar_search(matrix, start_point):
    ...
    goal_point = [dim - 1, dim - 1]
    queue = Prior_Queue()
    queue.add({'pos': start_point, 'dist': 0, 'heur': 2 * (dim - 1) - start_point[0] - start_point[1],
               'prior': 2 * (dim - 1) - start_point[0] - start_point[1]})
    history = []
    flag_shortest_path = 0
    parents_dict = {}
    while len(queue):
        ...
        for i in next_choice:
            ...
            if i == goal_point:
                ...
                flag_shortest_path = 1
                break
            queue.add({'pos': i, 'dist': current_dist + 1, 'heur': 2 * (dim - 1) - i[0] - i[1],
                       'prior': (2 * (dim - 1) - i[0] - i[1] + current_dist + 1)})
        if flag_shortest_path == 1:
            break
    if flag_shortest_path == 1:
        ### refind the path
...
def strategy_one(maze_init_, advanced_maze):
    '''
    find the shortest path (Astar search) of the initial maze, and ignore the fire advance.
    :param maze_init:
    :param advanced_maze:
    :return:
    '''
...
def strategy_two(maze_init_, advanced_maze):
    '''
    recompute the shortest path (Astar search) of the current maze at each step.
    :param maze_init:
    :param advanced_maze:
    :return:
    '''
...
def strategy_three(maze_init_, advanced_maze):
    '''
    using A star method, consider on step further than strategy 2
    :param maze_init:
    :param advanced_maze:
    :return:
    '''
...
def compare_method(fn1, fn2, fn3, maze, q):
    ...
    l = [0, 0, 0, 0]
    if fn1(maze,  advanced_maze):
        l[0] += 1
    if fn2(maze,  advanced_maze):
        l[1] += 1
    if fn3(maze,  advanced_maze):
        l[2] += 1

    return l

@timer(True)
def plot_success_vs_flammability_multi_thread(fn1, fn2, fn3, dim=30, p=0.3, num_maze=10, cnt_initial_fire=10,
    ...
    plt.title('average successes vs flammability q, dim = %d' % (dim))
