...
def debug_DFS_BFS_Astar_and_generate(dim=30,repeat_nums = 10):
    ...
    for _ in tqdm(range(repeat_nums)):
        ...
        bfs_t += t2 - t1
        ast_t += time.time() - t2
    print('dfs time:', dfs_t/repeat_nums*1000, 'bfs time:', bfs_t/repeat_nums*1000, 'astar time: ', ast_t/repeat_nums*1000)
...
def compare_startegy_time_cost(dim=10, p=0.3, q=0.5, num_maze=10, cnt_initial_fire=1):
    ...
    for _ in tqdm(range(num_maze)):
        ...
        t0 = time.time()
        strategy_one(maze, q, advanced_maze)
        t1 = time.time()
        strategy_two(maze, q, advanced_maze)
        t2 = time.time()
        strategy_three(maze, q, advanced_maze)
        t3 = time.time()
        s1 += t1 - t0
        s2 += t2 - t1
        s3 += t3 - t2
...
if __name__ == '__main__':
    ...
    debug_DFS_BFS_Astar_and_generate(dim=20,repeat_nums=100)
    # debug_generate_maze()
    # debug_advanced_maze()
